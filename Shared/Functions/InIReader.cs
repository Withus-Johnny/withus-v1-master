...
public class InIReader
{
    ...
    private readonly char _equalChar = '=';
    private readonly string _equalStr = "=";

    // Square Brackets
    private readonly string _openSB = "[";
    private readonly string _closeSB = "]";
    ...
    private string FindValue(string section, string key)
    {
        for (int a = 0; a < _contents.Count; a++)
            if (String.CompareOrdinal(_contents[a], _openSB + section + _closeSB) == 0)
                for (int b = a + 1; b < _contents.Count; b++)
                    if (String.CompareOrdinal(_contents[b].Split(_equalChar)[0], key) == 0)
                        return _contents[b].Split(_equalChar)[1];
                    else if (_contents[b].StartsWith(_openSB) && _contents[b].EndsWith(_closeSB))
                        return null;
        return null;
    }
        ...
        for (int a = 0; a < _contents.Count; a++)
            if (String.CompareOrdinal(_contents[a], _openSB + section + _closeSB) == 0)
                for (int b = a + 1; b < _contents.Count; b++)
                    ...
                    {
                        _contents.Insert(b - 1, key + _equalStr);
                        return b - 1;
                    }
                    else if (_contents.Count - 1 == b)
                    {
                        _contents.Add(key + _equalStr);
                        return _contents.Count - 1;
                    }
        if (_contents.Count > 0)
            _contents.Add("");

        _contents.Add(_openSB + section + _closeSB);
        _contents.Add(key + _equalStr);
        return _contents.Count - 1;
    }
    ...
    public Point ReadPoint(string section, string key, Point Default)
    {
        ...
        int tempX, tempY;
        if (temp == null || !int.TryParse(temp.Split(_separatorChar)[0], out tempX))
        {
            Write(section, key, Default);
            return Default;
        }
        if (!int.TryParse(temp.Split(_separatorChar)[1], out tempY))
        {
            Write(section, key, Default);
            return Default;
        }
    ...
    public Size ReadSize(string section, string key, Size Default)
    {
        string temp = FindValue(section, key);
        int tempX, tempY;
        if (!int.TryParse(temp.Split(_separatorChar)[0], out tempX))
        {
            Write(section, key, Default);
            return Default;
        }
    ...
    {
        _contents[FindIndex(section, key)] = key + _equalStr + value;
        Save();
    }
    ...
    {
        _contents[FindIndex(section, key)] = key + _equalStr + value.X + _separatorStr + value.Y;
        Save();
    }

    public void Write(string section, string key, Size value)
    {
        _contents[FindIndex(section, key)] = key + _equalStr + value.Width + _separatorStr + value.Height;
        Save();
    }
